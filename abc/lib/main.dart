...
import 'appointmentEdior.dart';

void main() => runApp(const MaterialApp(home: ShiftScheduler()));
...
class _ShiftSchedulerState extends State<ShiftScheduler> {
  ...
  late DateTime fristTime;
  late DateTime secondTime;
  late dynamic beforeEdit;
  late CalendarResource beforEditCalendarResource;
  ...
  void dragStart(AppointmentDragStartDetails appointmentDragStartDetails) {
    ...
    beforeEdit = appointment!;
    fristTime = appointment.startTime;
  ...
  void dragUpdate(AppointmentDragUpdateDetails appointmentDragUpdateDetails) {
    ...
    final Offset? draggingOffset =
        appointmentDragUpdateDetails.draggingPosition;
  ...
  void dragEnd(AppointmentDragEndDetails appointmentDragEndDetails) {
    ...
    // appointment = beforeEdit;
    var targetResource = appointmentDragEndDetails.targetResource;
    var sourceResource = appointmentDragEndDetails.sourceResource;
    secondTime = appointmentDragEndDetails.droppingTime!;
    _showDialog(appointment, beforeEdit, targetResource!, sourceResource!,
  ...
  void _showDialog(
      ...
      DateTime fristTime,
      DateTime secondTime) async {
    await showDialog(
      builder: (context) => AlertDialog(
        title: const Text("Dropped resource details"),
        contentPadding: const EdgeInsets.all(16.0),
        content: Text("You have dropped The " +
            DateFormat('hh:mm:s').format(fristTime) +
            " to " +
            DateFormat('hh:mm:s').format(secondTime) +
            "\n" +
            "And To human ID : " +
          ...
          TextButton(
              ...
              onPressed: () {
                setState(() {
                  print("-");
                  print(appointment);
                  // beforeEdit.resoureIds = sourceResource.id;

                  // xoas

                  _events.appointments!
                      .removeAt(_events.appointments!.indexOf(appointment));
                  ...
                  Appointment old = Appointment(
                        startTime: beforeEdit.startTime,
                        endTime: beforeEdit.endTime,
                        color: beforeEdit.color,
                        startTimeZone: beforeEdit.startTimeZone,
                        endTimeZone: beforeEdit.endTimeZone,
                        notes: beforeEdit.notes,
                        isAllDay: beforeEdit.isAllDay,
                        location: beforeEdit.location,
                        subject: beforeEdit.subject,
                        resourceIds: [sourceResource.id],
                        id: beforeEdit.id,
                        recurrenceId: beforeEdit.recurrenceId,
                  );

                  print(old);

                  (_events.appointments!.add(old));

                  _events.notifyListeners(
                      CalendarDataSourceAction.add, <Appointment>[old]);
  ...
  void _onCalendarTapped(CalendarTapDetails calendarTapDetails) {
    ...
    if (true && _calendarController.view == CalendarView.month) {
